...
dt_ioi_subset_all <- dt_ioi[Subcomponent != "NA" & !is.na(normIOI)]

# draw histogram and boxplot
p_ioi_hist <- ggplot(dt_ioi_subset_all, aes(x = normIOI, fill = Grouping)) +
  geom_histogram(position = "identity", alpha = .5, binwidth = .01)
plot(p_ioi_hist)

p_ioi_box <- ggboxplot(dt_ioi_subset_all, x = "Skill", y = "normIOI", color = "Condition")
