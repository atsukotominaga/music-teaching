...
for (row in 1:nrow(dt_error_onset)){
  current <- dt_onset[SubNr == t_error_onset$SubNr[1] & BlockNr == dt_error_onset$BlockNr[2] & TrialNr == dt_error_onset$TrialNr[3]]
  if (nrow(current) > nrow(dt_ideal)){ # extra notes
    
  } else if (nrow(current) < nrow(dt_ideal)){ # missing notes
    
  } else if (nrow(current) == nrow(dt_ideal)){ # substituted notes or something else
