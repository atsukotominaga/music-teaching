...
# 3_stats/vel_art - store csv files
if (!file.exists("3_stats/vel_art")){
  dir.create("3_stats/vel_art")
}
...
# 3_stats/vel_art - store png files
if (!file.exists("3_stats/plot/vel_art")){
  dir.create("3_stats/plot/vel_art")
}
...
df_vel <- df_vel %>% dplyr::filter(Skill == "articulation")
df_vel_acc <- df_vel_acc %>% dplyr::filter(Skill == "articulation")
...
# Velocity plots
####################################
p_vel_sub <- ggplot(data = vel_sub_stats, aes(x = SubSkill, y = Mean, fill = Condition)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = Mean - SEM, ymax = Mean + SEM),
                width=.2, position = position_dodge(.9)) +
  labs(x = "SubSkill", y = "Velocity (0-127)") + coord_cartesian(ylim = c(50, 80)) + 
...
p_vel_ch_sub <- ggplot(data = vel_ch_sub_stats, aes(x = SubSkill, y = Mean, fill = Condition)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = Mean - SEM, ymax = Mean + SEM),
                width=.2, position = position_dodge(.9)) +
...
p_vel_phrase <- ggplot(data = vel_phrase_stats, aes(x = SubSkill, y = Mean, fill = Condition)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = Mean - SEM, ymax = Mean + SEM),
                width=.2, position = position_dodge(.9)) +
  facet_grid(. ~ reorder(Boundary, LabelOrder)) +
  labs(x = "SubSkill", y = "Velocity (0-127)") +
  theme_classic()
p_vel_phrase

# Save plots
# png files
ggsave("./3_stats/plot/vel_art/p_vel_sub.png", plot = p_vel_sub, dpi = 600, width = 5, height = 4)
ggsave("./3_stats/plot/vel_art/p_vel_ch_sub.png", plot = p_vel_ch_sub, dpi = 600, width = 5, height = 4)
ggsave("./3_stats/plot/vel_art/p_vel_seq.png", plot = p_vel_seq, dpi = 600, width = 15, height = 4)
ggsave("./3_stats/plot/vel_art/p_vel_acc_seq.png", plot = p_vel_acc_seq, dpi = 600, width = 15, height = 4) 
ggsave("./3_stats/plot/vel_art/p_vel_phrase.png", plot = p_vel_phrase, dpi = 600, width = 7, height = 4)
...
print(vel_sub_ph)
write.csv(vel_sub_ph$`Condition:SubSkill`, file = "./3_stats/vel_art/vel_sub_ph.csv")
...
print(vel_ch_sub_ph)
write.csv(vel_ch_sub_ph$`Condition:SubSkill`, file = "./3_stats/vel_art/vel_ch_sub_ph.csv")
