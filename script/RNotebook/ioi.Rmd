...
print(ioi_art_var_aov)
```

##### Two-way ANOVA (aov)
```{r stats6, echo = FALSE}
ioi_art_var_aov_2 <- with(ioi_art_var,
  aov(Mean ~ Condition + Error(SubNr/Condition)))
print(summary(ioi_art_var_aov_2))
```
...
##### Two-way ANOVA (ez)
```{r stats7, echo = FALSE, message = FALSE}
ioi_dyn_aov <- ezANOVA(
  data = df_ioi_dyn
  , dv = .(IOI)
  , wid = .(SubNr)
...
##### Two-way ANOVA (aov)
```{r stats8, echo = FALSE, message = FALSE}
ioi_dyn_aov_2 <- aov(Mean ~ Condition + Error(SubNr/Condition), data = ioi_dyn)
print(summary(ioi_dyn_aov_2))
```
...
print(ioi_dyn_var_aov)
```

##### Two-way ANOVA (aov)
```{r stats12, echo = FALSE}
ioi_dyn_var_aov_2 <- with(ioi_dyn_var,
  aov(Mean ~ Condition + Error(SubNr/Condition)))
print(summary(ioi_dyn_var_aov_2))
```
