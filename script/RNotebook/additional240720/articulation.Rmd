...
Description: This is a summary of all analyses for articulation. **Add excluded 14 participants.**
...
filename_ioi = "./trimmed/data_ioi.csv"
filename_kot = "./trimmed/data_kot.csv"
```
...
for (subnr in unique(df_kot$SubNr)){
  for (block in unique(df_kot$BlockNr[df_kot$SubNr == subnr])){
    for (trial in unique(df_kot$TrialNr[df_kot$SubNr == subnr & df_kot$BlockNr == block])){
      if (length(ioi$Mean[ioi$SubNr == subnr & ioi$BlockNr == block & ioi$TrialNr == trial]) != 0){ # exclude if mean IOI does not exist
...
# Group mean
kot_dyn_all <- aggregate(Mean~Condition*Skill*Subcomponent, data = subset(kot_dyn, kot_dyn$SubNr != 3 & kot_dyn$SubNr != 14 & kot_dyn$SubNr != 31), # exclude SubNr 3 & 14 & 31 as one of the conditions is completely missing
                         FUN = function(x){round(c(N = length(x), mean = mean(x), sd = sd(x), sem = sd(x)/sqrt(length(x))), 4)})
...
#### Two-way ANOVA (ez)
```{r stats5, echo = FALSE}
# exclude SubNr 3 & 14 & 31
df_kot_dyn_exc <- subset(df_kot_dyn, df_kot_dyn$SubNr != 3 & df_kot_dyn$SubNr != 14 & df_kot_dyn$SubNr != 31)
kot_dyn_aov <- ezANOVA(
  data = subset(df_kot_dyn_exc, df_kot_dyn_exc$Subcomponent == "Forte" | df_kot_dyn_exc$Subcomponent == "Piano")
  , dv = .(KOT)
  , wid = .(SubNr)
  , within = .(Condition, Subcomponent)
  , type = 3
...
#### Two-way ANOVA (aov)
```{r stats6, echo = FALSE}
kot_dyn_aov_2 <- aov(Mean ~ Condition*Subcomponent + Error(SubNr/(Condition*Subcomponent)), data = subset(kot_dyn, kot_dyn$SubNr != 3 & kot_dyn$SubNr != 14 & kot_dyn$SubNr != 31))
