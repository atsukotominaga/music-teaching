...
# Exclude SubNr 1 as data for the teaching condition were missing
# Group mean
ioi_art_ch_all <- aggregate(Mean~Condition*Skill*Boundary, data = subset(ioi_art_ch, ioi_art_ch$SubNr != 1),
                       FUN = function(x){round(c(N = length(x), mean = mean(x), sd = sd(x), sem = sd(x)/sqrt(length(x))), 4)})
...
ioi_art_ch_aov <- ezANOVA(
  data = subset(df_ioi_art, df_ioi_art$SubNr != 1)
  , dv = .(IOI)
  , wid = .(SubNr)
...
#### Two-way ANOVA (aov)
```{r stats4, echo = FALSE}
ioi_art_ch_aov_2 <- aov(Mean ~ Condition*Boundary + Error(SubNr/(Condition*Boundary)), data = subset(ioi_art_ch, ioi_art_ch$SubNr != 1))
...
# Exclude SubNr 3 and 14 as data were missing for one of the conditions
# Group mean
ioi_dyn_all <- aggregate(Mean~Condition*Skill, data = subset(ioi_dyn, ioi_dyn$SubNr != 3 & ioi_dyn$SubNr != 14),
                       FUN = function(x){round(c(N = length(x), mean = mean(x), sd = sd(x), sem = sd(x)/sqrt(length(x))), 4)})
...
# Exclude SubNr 3, 14 as data were missing for one of the conditions.
# Group mean
ioi_dyn_ch_all <- aggregate(Mean~Condition*Skill*Boundary, data = subset(ioi_dyn_ch, ioi_dyn_ch$SubNr != 3 & ioi_dyn_ch$SubNr != 14),
                       FUN = function(x){round(c(N = length(x), mean = mean(x), sd = sd(x), sem = sd(x)/sqrt(length(x))), 4)})
...
ioi_dyn_ch_aov <- ezANOVA(
  data = subset(df_ioi_dyn, df_ioi_dyn$SubNr != 3 & df_ioi_dyn$SubNr != 14)
  , dv = .(IOI)
  , wid = .(SubNr)
  , within = .(Condition, Boundary)
  , type = 3
...
ioi_dyn_ch_aov_2 <- aov(Mean ~ Condition*Boundary + Error(SubNr/(Condition*Boundary)), data = subset(ioi_dyn_ch, ioi_dyn_ch$SubNr != 3 & ioi_dyn_ch$SubNr != 14))
