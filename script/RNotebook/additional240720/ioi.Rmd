...
Description: This is a summary of all analyses for a tempo. **Add excluded 14 participants.**
...
```{r trans_art, echo = FALSE}
# transformation
ioi_art_trans <- ioi_art
ioi_art_trans$Mean <- 1/(ioi_art$Mean)

# normality check
ioi_art_trans_norm <- by(ioi_art_trans$Mean, list(ioi_art_trans$Condition), shapiro.test)
print(ioi_art_trans_norm)

# use transformed data 
ioi_art <- ioi_art_trans
```
...
```
#### Non-parametric test (Wilcoxon-rank)
```{r non-para_art, echo = FALSE}
# For each individual
n_ioi_art <- aggregate(IOI~SubNr*Condition*Skill, data = df_ioi_art,
                 FUN = function(x){c(length(x), mean = mean(x), sd = sd(x))})
n_ioi_art <- cbind(n_ioi_art[,1:3], as.data.frame(n_ioi_art[,4]))
# Change colnames
colnames(n_ioi_art) <- c("SubNr", "Condition", "Skill", "N", "Mean", "SD")

nonpara_art <- wilcox.test(Mean ~ Condition, data = n_ioi_art, paired = TRUE)
print(nonpara_art)
```
...
<!-- # Create a data frame for Variability (SD/mean IOI) -->
...
# Group mean
ioi_dyn_all <- aggregate(Mean~Condition*Skill, data = subset(ioi_dyn, ioi_dyn$SubNr != 3 & ioi_dyn$SubNr != 14), # exclude SubNr 3 & 14 as they missed one of the conditions completely.
                       FUN = function(x){round(c(N = length(x), mean = mean(x), sd = sd(x), sem = sd(x)/sqrt(length(x))), 4)})
...
```{r trans_dyn, echo = FALSE}
# transformation
ioi_dyn_trans <- ioi_dyn
ioi_dyn_trans$Mean <- 1/(ioi_dyn$Mean)

# normality check
ioi_dyn_trans_norm <- by(ioi_dyn_trans$Mean, list(ioi_dyn_trans$Condition), shapiro.test)
print(ioi_dyn_trans_norm)

# use transformed data 
ioi_dyn <- ioi_dyn_trans
```
...
#### Non-parametric test (Wilcoxon-rank)
```{r non-para_dyn, echo = FALSE}
# For each individual
n_ioi_dyn <- aggregate(IOI~SubNr*Condition*Skill, data = subset(df_ioi_dyn, df_ioi_dyn$SubNr != 3 & df_ioi_dyn$SubNr != 14),
                 FUN = function(x){c(length(x), mean = mean(x), sd = sd(x))})
n_ioi_dyn <- cbind(n_ioi_dyn[,1:3], as.data.frame(n_ioi_dyn[,4]))
# Change colnames
colnames(n_ioi_dyn) <- c("SubNr", "Condition", "Skill", "N", "Mean", "SD")

nonpara_dyn <- wilcox.test(Mean ~ Condition, data = n_ioi_dyn, paired = TRUE)
print(nonpara_dyn)
```
...
<!-- # IOI-Order-Articulation -->
<!-- - Investigate whether there is any order effect of the conditions. -->

<!-- ## 1. All{#art_order_group_all} -->
<!-- ```{r ioi_art_order_all, echo = FALSE} -->
<!-- # For each participant -->
<!-- ioi_art_order <- aggregate(IOI~SubNr*Condition*Skill*Order, data = df_ioi_art, -->
<!--                                      FUN = function(x){round(c(N = length(x), mean = mean(x), sd = sd(x)), 4)}) -->
<!-- ioi_art_order <- cbind(ioi_art_order[,1:4], as.data.frame(ioi_art_order[,5])) -->
<!-- # Change colnames -->
<!-- colnames(ioi_art_order) <- c("SubNr", "Condition", "Skill", "Order", "N", "Mean", "SD") -->

<!-- # Group mean -->
<!-- ioi_art_order_all <- aggregate(Mean~Condition*Skill*Order, data = ioi_art_order, -->
<!--                                      FUN = function(x){round(c(N = length(x), mean = mean(x), sd = sd(x), sem = sd(x)/sqrt(length(x))), 4)}) -->
<!-- ioi_art_order_all <- cbind(ioi_art_order_all[,1:3], as.data.frame(ioi_art_order_all[,4])) -->
<!-- # Change colnames -->
<!-- colnames(ioi_art_order_all) <- c("Condition", "Skill", "Order", "N", "Mean", "SD", "SEM") -->
<!-- print(ioi_art_order_all) -->
<!-- ``` -->
...
<!-- ```{r ioi_art_order_all_bar, echo = FALSE} -->
<!-- p_ioi_art_order_all <- ggplot(data = ioi_art_order_all, aes(x = Condition, y = Mean, fill = Condition)) + -->
<!--   geom_bar(stat = "identity", position = position_dodge()) + -->
<!--   geom_errorbar(aes(ymin = Mean - SEM, ymax = Mean + SEM), -->
<!--                 width = .2, position = position_dodge()) + -->
<!--   geom_hline(yintercept = 188, linetype = "dashed") + # Tempo -->
...
<!-- ```{r ioi_art_order_all_box, echo = FALSE} -->
<!-- p_ioi_art_order_all_box <- ggplot(data = ioi_art_order, aes(x = Condition, y = Mean, fill = Condition)) + -->
<!--   geom_boxplot() + -->
<!--   geom_dotplot(binaxis = "y", stackdir = "center", dotsize = 1) + -->
<!--   geom_hline(yintercept = 188, linetype = "dashed") + # Tempo -->
<!--   facet_wrap(Order ~ .) + -->
...
<!-- # Normality check -->
<!-- ioi_art_order_norm <- by(ioi_art_order$Mean, list(ioi_art_order$Condition, ioi_art_order$Order), shapiro.test) -->
<!-- print(ioi_art_order_norm) -->
<!-- ``` -->

<!-- #### Two-way ANOVA (ez) -->
<!-- ```{r stats13, echo = FALSE} -->
<!-- ioi_art_order_aov <- ezANOVA( -->
<!--   data = df_ioi_art -->
<!--   , dv = .(IOI) -->
<!--   , wid = .(SubNr) -->
<!--   , between = .(Order) -->
...
<!--   , detailed = TRUE -->
<!-- ) -->
<!-- ioi_art_order_aov$ANOVA$F <- round(ioi_art_order_aov$ANOVA$F, 4) -->
<!-- ioi_art_order_aov$ANOVA$p <- round(ioi_art_order_aov$ANOVA$p, 4) -->
<!-- print(ioi_art_order_aov) -->
<!-- ``` -->

<!-- #### Two-way ANOVA (aov) -->
<!-- ```{r stats14, echo = FALSE} -->
<!-- ioi_art_order_aov_2 <- aov(Mean ~ Condition*Order + Error(SubNr/Condition) + Order, data = ioi_art_order) -->
<!-- print(summary(ioi_art_order_aov_2)) -->
<!-- ``` -->

<!-- # IOI-Order-Dynamics -->
<!-- - Investigate whether there is any order effect of the conditions. -->

<!-- ## 1. All{#dyn_order_group_all} -->
<!-- ```{r ioi_dyn_order_all, echo = FALSE} -->
<!-- # For each pdynicipant -->
<!-- ioi_dyn_order <- aggregate(IOI~SubNr*Condition*Skill*Order, data = df_ioi_dyn, -->
<!--                                      FUN = function(x){round(c(N = length(x), mean = mean(x), sd = sd(x)), 4)}) -->
<!-- ioi_dyn_order <- cbind(ioi_dyn_order[,1:4], as.data.frame(ioi_dyn_order[,5])) -->
<!-- # Change colnames -->
<!-- colnames(ioi_dyn_order) <- c("SubNr", "Condition", "Skill", "Order", "N", "Mean", "SD") -->

<!-- # Group mean -->
<!-- ioi_dyn_order_all <- aggregate(Mean~Condition*Skill*Order, data = ioi_dyn_order, -->
<!--                                      FUN = function(x){round(c(N = length(x), mean = mean(x), sd = sd(x), sem = sd(x)/sqrt(length(x))), 4)}) -->
<!-- ioi_dyn_order_all <- cbind(ioi_dyn_order_all[,1:3], as.data.frame(ioi_dyn_order_all[,4])) -->
<!-- # Change colnames -->
<!-- colnames(ioi_dyn_order_all) <- c("Condition", "Skill", "Order", "N", "Mean", "SD", "SEM") -->
<!-- print(ioi_dyn_order_all) -->
<!-- ``` -->
...
<!-- ```{r ioi_dyn_order_all_bar, echo = FALSE} -->
<!-- p_ioi_dyn_order_all <- ggplot(data = ioi_dyn_order_all, aes(x = Condition, y = Mean, fill = Condition)) + -->
<!--   geom_bar(stat = "identity", position = position_dodge()) + -->
<!--   geom_errorbar(aes(ymin = Mean - SEM, ymax = Mean + SEM), -->
<!--                 width = .2, position = position_dodge()) + -->
<!--   geom_hline(yintercept = 188, linetype = "dashed") + # Tempo -->
...
<!-- ```{r ioi_dyn_order_all_box, echo = FALSE} -->
<!-- p_ioi_dyn_order_all_box <- ggplot(data = ioi_dyn_order, aes(x = Condition, y = Mean, fill = Condition)) + -->
<!--   geom_boxplot() + -->
<!--   geom_dotplot(binaxis = "y", stackdir = "center", dotsize = 1) + -->
<!--   geom_hline(yintercept = 188, linetype = "dashed") + # Tempo -->
<!--   facet_wrap(Order ~ .) + -->
...
<!-- # Normality check -->
<!-- ioi_dyn_order_norm <- by(ioi_dyn_order$Mean, list(ioi_dyn_order$Condition, ioi_dyn_order$Order), shapiro.test) -->
<!-- print(ioi_dyn_order_norm) -->
<!-- ``` -->

<!-- #### Two-way ANOVA (ez) -->
<!-- ```{r stats15, echo = FALSE} -->
<!-- ioi_dyn_order_aov <- ezANOVA( -->
<!--   data = df_ioi_dyn -->
<!--   , dv = .(IOI) -->
<!--   , wid = .(SubNr) -->
<!--   , between = .(Order) -->
...
<!--   , detailed = TRUE -->
<!-- ) -->
<!-- ioi_dyn_order_aov$ANOVA$F <- round(ioi_dyn_order_aov$ANOVA$F, 4) -->
<!-- ioi_dyn_order_aov$ANOVA$p <- round(ioi_dyn_order_aov$ANOVA$p, 4) -->
<!-- print(ioi_dyn_order_aov) -->
<!-- ``` -->

<!-- #### Two-way ANOVA (aov) -->
<!-- ```{r stats16, echo = FALSE} -->
<!-- ioi_dyn_order_aov_2 <- aov(Mean ~ Condition*Order + Error(SubNr/Condition) + Order, data = ioi_dyn_order) -->
<!-- print(summary(ioi_dyn_order_aov_2)) -->
<!-- ``` -->
...
# For each individual
ioi_art_seq <- aggregate(IOI~SubNr*Condition*Skill*Interval, data = df_ioi_art,
                                 FUN = function(x){c(N = length(x), mean = mean(x), sd = sd(x))})
...
# For each individual
ioi_dyn_seq <- aggregate(IOI~SubNr*Condition*Skill*Interval, data = df_ioi_dyn,
                                 FUN = function(x){c(N = length(x), mean = mean(x), sd = sd(x))})
