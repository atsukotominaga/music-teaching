...
# File location
filename = "../../R/filtered/data_analysis.csv" # clear data without pitch errors
filename_exc = "../../R/filtered/data_errorRate.csv" # exclusion criteria

# Read filtered csv files
df_all <- read.csv(filename, header = T, sep = ",", dec = ".") 
df_exc <- read.csv(filename_exc, header = T, sep = ",", dec = ".") 

# Exclude participants with more than 10% errors
# include <- df_exc$SubNr[df_exc$LessThan10 == "include"]
include <- df_exc$SubNr # include all
...
write.csv(df_exc, file = "./trimmed/data_errorRate.csv", row.names = F)
...
ggsave("./trimmed/plot/ioi/p_box.png", plot = p_box, dpi = 600, width = 5, height = 4)
...
####################################
df_exc <- read.csv("./trimmed/data_errorRate.csv", header = T, sep = ",", dec = ".") # exclusion criteria
...
p_box_acc <- ggboxplot(df_subset_acc, x = "Subcomponent", y = "Velocity", color = "Condition")
