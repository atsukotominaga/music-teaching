...
Description: This is a summary of all analyses for dynamics. **Add excluded 14 participants.**
...
filename_vel = "./trimmed/data_vel.csv"
filename_vel_acc = "./trimmed/data_vel_acc.csv"
```
...
# Group mean
vel_dyn_all <- aggregate(Mean~Condition*Skill*Subcomponent, data = subset(vel_dyn, vel_dyn$SubNr != 3 & vel_dyn$SubNr != 14), # exclude SubNr 3 & 14 as one of the conditions is completely missing
...
p_vel_dyn_all_box <- ggplot(data = subset(vel_dyn, vel_dyn$SubNr != 3 & vel_dyn$SubNr != 14), aes(x = Subcomponent, y = Mean, fill = Condition)) +
  geom_boxplot() +
...
# exclude SubNr 3 & 14
vel_dyn <- subset(vel_dyn, vel_dyn$SubNr != 3 & vel_dyn$SubNr != 14)
...
```{r stats1, echo = FALSE}
vel_dyn_aov <- ezANOVA(
  data = subset(df_vel_dyn, df_vel_dyn$SubNr != 3 & df_vel_dyn$SubNr != 14)
  , dv = .(Velocity)
  , wid = .(SubNr)
...
# Group mean
vel_acc_dyn_all <- aggregate(Mean~Condition*Skill*Subcomponent, data = subset(vel_acc_dyn, vel_acc_dyn$SubNr != 3 & vel_acc_dyn$SubNr != 14), # exclude SubNr 3 & 14 as one of the conditions is completely missing
                         FUN = function(x){round(c(N = length(x), mean = mean(x), sd = sd(x), sem = sd(x)/sqrt(length(x))), 4)})
...
p_vel_acc_dyn_all_box <- ggplot(data = subset(vel_acc_dyn, vel_acc_dyn$SubNr != 3 & vel_acc_dyn$SubNr != 14), aes(x = Subcomponent, y = Mean, fill = Condition)) +
  geom_boxplot() +
  geom_dotplot(binaxis = "y", stackdir = "center", dotsize = 1, position = position_dodge(0.75)) +
  labs(y = "Difference", subtitle = "KV-Diff: Dynamics")
...
# exclude SubNr 3 & 14
vel_acc_dyn <- subset(vel_acc_dyn, vel_acc_dyn$SubNr != 3 & vel_acc_dyn$SubNr != 14)
# Reduce unused factors
...
#### Two-way ANOVA (ez)
```{r stats3, echo = FALSE}
df_vel_acc_dyn_exc <- subset(df_vel_acc_dyn, df_vel_acc_dyn$SubNr != 3 & df_vel_acc_dyn$SubNr != 14)
vel_acc_dyn_aov <- ezANOVA(
  data = subset(df_vel_acc_dyn_exc, df_vel_acc_dyn_exc$Subcomponent == "FtoP" | df_vel_acc_dyn_exc$Subcomponent == "PtoF")
  , dv = .(Acc)
  , wid = .(SubNr)
  , within = .(Condition, Subcomponent)
  , type = 3
