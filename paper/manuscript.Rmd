...
In order to compare the mean IOIs between the teaching and performing condition, we conducted a Wiscoxon Signed-rank test, instead of a paired *t*-test, because a Shapiro-Wilk test showed that the distribution of the mean difference was significantly different from the normal distribution (*p* `r if(ioi_dyn_norm$p.value < 0.001){"< 0.001"} else {paste("= ", sprintf('%.3f', ioi_dyn_norm$p.value))}`). The Wiscoxon Signed-rank test revealed no significant difference between the teaching condition [*Mdn* = `r ioi_dyn_all[Condition == "teaching"]$Median` (ms), *IQR* = `r ioi_dyn_all[Condition == "teaching"]$IQR`] and the performing condition [*Mdn* = `r ioi_dyn_all[Condition == "performing"]$Median` (ms), *IQR* = `r ioi_dyn_all[Condition == "performing"]$IQR`] while playing the piece with the notated dynamics (*p* `r if(ioi_dyn_wilcoxon$p.value < 0.001){"< 0.001"} else {paste("= ", sprintf('%.2f', ioi_dyn_wilcoxon$p.value))}`; *Figure \ref{fig:ioi-1}*).
...
# annotate * for forte
img_vel_dyn_1 <- image_annotate(img_vel_dyn_1, "*", size = 280, color = "black", location = "+1430+800")
```
...
vel_diff_dyn_1 <- ggpaired(vel_diff_dyn_plot, cond = "performing", cond2 = "teaching", color = "condition", facet.by = "subcomponent", line.size = 0.3, line.color = "gray", palette = "aaas", xlab = "Condition", ylab = "Difference", title = "KV-Diff: Exp 1") +
  ylim(-40, 60) +
  stat_compare_means(label = "p.signif", hide.ns = TRUE, size = 10, label.x = 1.4, label.y = 45, method = "t.test", paired = TRUE) +
  theme_pubr(base_size = 20, legend = "none", base_family = "Helvetica")
...
A two-way repeated-measures ANOVA with the factors Condition (teaching vs. performing) and Subcomponent-FP (forte vs. piano) revealed that there was a significant main effect of Subcomponent-FP (*F*(`r kot_dyn_aov$anova_table[2,1]`,`r kot_dyn_aov$anova_table[2,2]`) = `r sprintf('%.0f', kot_dyn_aov$anova_table[2,4])`, *p* `r if(kot_dyn_aov$anova_table[2,6] < 0.001){"< 0.001"} else {paste("= ", sprintf('%.3f', kot_dyn_aov$anova_table[2,6]))}`, $\eta_G^2$ = `r sprintf('%.2f', kot_dyn_aov$anova_table[2,5])`; *Figure \ref{fig:kot-1}*), reflecting more key overlap for forte notes compared to piano notes. Neither the main effect of Condition (*F*(`r kot_dyn_aov$anova_table[1,1]`,`r kot_dyn_aov$anova_table[1,2]`) = `r sprintf('%.2f', kot_dyn_aov$anova_table[1,4])`, *p* `r if(kot_dyn_aov$anova_table[1,6] < 0.001){"< 0.001"} else {paste("= ", sprintf('%.2f', kot_dyn_aov$anova_table[1,6]))}`, $\eta_G^2$ = `r sprintf('%.2f', kot_dyn_aov$anova_table[1,5])`) nor the interaction between Condition and Subcomponent-FP (*F*(`r kot_dyn_aov$anova_table[3,1]`,`r kot_dyn_aov$anova_table[3,2]`) = `r sprintf('%.2f', kot_dyn_aov$anova_table[3,4])`, *p* `r if(kot_dyn_aov$anova_table[3,6] < 0.001){"< 0.001"} else {paste("= ", sprintf('%.2f', kot_dyn_aov$anova_table[3,6]))}`, $\eta_G^2$ = `r sprintf('%.2f', kot_dyn_aov$anova_table[3,5])`) was significant.
...
We recruited 21 piano experts who already had a degree in (above bachelor or equivalent) in piano performance/teaching or were studying advanced piano performance at a music school at the time of recruitment. For data analysis, we excluded one participants due to insufficient motor skills, and one because of large tempo deviations (outside 2 standard deviations from the average tempo across participants). Twenty participants (`r nrow(dt_2_included[Gender == "Female"])` female) were included in the data analysis. Most participants were right-handed (left: `r nrow(dt_2_included[Handedness == "Left"])`) with a mean age of `r round(mean(dt_2_included$Age), 2)` (*SD* = `r round(sd(dt_2_included$Age), 2)`). They had `r round(mean(dt_2_included$PianoTotalPractice), 2)` years of practice on average (*SD* = `r round(sd(dt_2_included$PianoTotalPractice), 2)`). 
...
kor_art_2 <- ggpaired(kor_art_plot, cond = "performing", cond2 = "teaching", color = "condition", facet.by = "subcomponent", line.size = 0.3, line.color = "gray", palette = "aaas", xlab = "Condition", ylab = "KOR", title = "KOR: Articulation") +
  ylim(-1, 0.4) +
  stat_compare_means(label = "p.signif", hide.ns = TRUE, size = 10, label.x = 1.4, label.y = -0.5, method = "t.test", paired = TRUE) +
  theme_pubr(base_size = 20, legend = "none", base_family = "Helvetica")
...
### 3.2.6. Dynamics: KV Transition Points
