...
A paired-sample \emph{t}-test showed that IOIs at transition points were larger for the teaching condition {[}\emph{M} = 1.00, \emph{SD} = 0.067{]} than the performing condition {[}\emph{M} = 0.98, \emph{SD} = 0.049{]} while playing the piece with the notated articulation (\emph{t}(19) = 2.27, \emph{p} = 0.03, Cohen's \emph{d} = 0.51, two-tailed, \emph{Fig \ref{fig:tra}}).
